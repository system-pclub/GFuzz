...
func (h *InterestHandlerImpl) HandleInterest(i *api.InterestInput) (ret bool, err error) {
	...
	case api.InitStage:
		if i.HandledCnt == 1 {
			...
			ret, err = handleInitStageInput(h.fctx, i.Input, i.Output)
		} else {
	...
	case api.DeterStage:
		...
		ret, err = handleDeterStageInput(h.fctx, i.Input, i.Output)
	case api.CalibStage:
		// we are handling the output from the input with calib stage
		ret, err = handleCalibStageInput(h.fctx, i.Input, i.Output)
	case api.RandStage:
